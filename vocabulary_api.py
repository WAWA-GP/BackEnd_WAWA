...
async def update_word_content(
        word_id: int,
        word_in: vocabulary_model.UserWordContentUpdate,
        db: AsyncClient = Depends(get_db),
        current_user: dict = Depends(get_current_user)
):
...
async def get_stats(
    ...
    user_id = current_user.get('user_id')
    return await vocabulary_service.get_word_stats(db, user_id)
...
async def get_all_words(
    ...
    user_id = current_user.get('user_id')
    return await vocabulary_service.get_all_words(db, user_id, status)
